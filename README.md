...
_Caption: Placeholder hero image used by each product card (IDs 1-3). Update these JPGs when brand assets are available._
...
_Caption: Component/context topology. `ProductProvider` seeds the catalog, while `CartProvider` exposes reducer actions consumed by `Product` and `CartLineItem`._
